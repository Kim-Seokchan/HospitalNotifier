...
**작업 지시사항:**
- `check_snuh.py`의 실행 흐름을 참고하여, 복잡한 서비스 로직을 명확한 상태 기반으로 재구성하고, 오류 처리 및 후속 동작(재로그인, 대기, 중지 등)을 체계적으로 개선
...
**작업 내용:**
- **로직 개선:**
    - `ReservationService.kt`의 로직을 상태 머신(State Machine) 패턴을 사용하여 재구성했습니다. `IDLE`, `LOGGING_IN`, `CHECKING`, `RE_LOGGING_IN`, `WAITING_FOR_RETRY`, `SUCCESS_AND_STOPPED` 등의 명확한 상태를 정의하고, 각 상태에 맞는 동작을 수행하도록 변경하여 코드의 가독성과 안정성을 높였습니다.
    - **로그인:** 서비스 시작 시 즉시 로그인을 시도하며, 실패 시 서비스가 중지됩니다.
    - **예약 조회:** 로그인 성공 후 조회를 시작하며, 예약 가능한 날짜가 없으면 설정된 주기에 따라 다음 조회를 예약합니다.
    - **성공 및 중지:** 예약 가능한 날짜를 찾으면, 로그 및 텔레그램 알림을 보낸 후 **서비스가 자동으로 중지**되어 더 이상 불필요한 작업을 수행하지 않습니다.
    - **오류 처리:** 세션 만료 등 조회 중 오류가 발생하면 자동으로 재로그인을 시도합니다.
    - **재로그인 실패 처리:** 재로그인에 실패할 경우, **15분 대기** 후 다시 재로그인을 시도하는 로직을 추가하여 안정성을 강화했습니다.

- **UI 편의성 개선:**
    - `activity_main.xml`의 텔레그램 토큰 및 챗 ID 입력 필드에 `autofillHints` 속성을 추가하여, 삼성 패스와 같은 자동 완성 서비스를 통해 사용자가 정보를 쉽게 입력하고 저장할 수 있도록 개선했습니다.

- **사용성 개선:**
    - **UI 상태 복원:** `MainActivity`가 다시 시작될 때, `SharedPreferences`에 저장된 사용자 입력(ID, PW, 조회 월 등)을 자동으로 복원하여 UI에 표시하도록 수정했습니다. 이를 통해 사용자가 앱을 나갔다 들어와도 입력 내용이 유지됩니다.
    - **지속적인 알림:** `ForegroundService`의 알림을 스와이프로 지울 수 없도록 `setOngoing(true)` 속성을 추가하여, 사용자가 실수로 서비스를 종료하는 것을 방지합니다.
    - **알림 내 종료 버튼:** 알림에 '종료' 버튼을 추가하여, 사용자가 명시적으로 서비스를 중지하고 모든 관련 데이터를 초기화할 수 있는 편리한 방법을 제공합니다.

- **알림 기능 개선:**
    - **알림 채널 분리:** 서비스의 지속적인 상태를 알리는 채널과 예약 성공을 알리는 채널을 분리했습니다.
    - **소음 최소화:** 지속적인 상태 알림은 소리가 나지 않는 낮은 중요도로 설정하여, 주기적인 작업마다 알림음이 울리는 문제를 해결했습니다.
    - **성공 시 명확한 알림:** 예약 성공 시에만 소리가 나는 높은 중요도의 알림이 별도로 표시되도록 하여, 사용자가 중요한 정보를 놓치지 않도록 개선했습니다.